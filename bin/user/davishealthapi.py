"""weewx module that records health information from a Davis weather station using the v2 API.
...
class GetAPIData:
    def get_data(station_id, api_key, api_secret, poll_interval):
        ...
        if not api_key or not station_id or not api_secret:
            logerr("DavisHealthAPI is missing a required parameter. Double-check your configuration file. key: %s secret: %s station ID: %s" % (api_key, api_secret, station_id))
            return packet
        ...
        apiurl = "https://api.weatherlink.com/v2/historic/%s?api-key=%s&start-timestamp=%s&end-timestamp=%s&api-signature=%s&t=%s" % (parameters["station-id"], parameters["api-key"], parameters["start-timestamp"], parameters["end-timestamp"], apiSignature, parameters["t"])
        ...
        apiurl = "https://api.weatherlink.com/v2/current/%s?api-key=%s&api-signature=%s&t=%s" % (parameters["station-id"], parameters["api-key"], apiSignature, parameters["t"])
        try:
        ...
        try:
            ...
            for i in range(7):
                if historical_data[i]['data'] and (historical_data[i]['data_structure_type'] == 11 or historical_data[i]['data_structure_type'] == 13):
            ...
            logerr("No valid API data recieved. Double-check API key/secret and station id. Error is: %s" % e)
        except IndexError as e:
            logerr("No valid data structure types found in API data. Error is: %s" % e)
        finally:
            return (packet)
...
class DavisHealthAPIDriver(AbstractDevice):
    ...
    def genLoopPackets(self):
        while True:
            packet = GetAPIData.get_data(self.station_id, self.api_key, self.api_secret, self.polling_interval)
            yield packet
...
class DavisHealthAPI(StdService):
    ...
    def __init__(self, engine, config_dict):
        ...
        d = config_dict.get('davishealthapi', {})
        self.max_age = weeutil.weeutil.to_int(d.get('max_age', 2592000))
        self.api_key = d.get('api_key', None)
        self.api_secret = d.get('api_secret', None)
        self.station_id = d.get('station_id', None)
        loginf("Collected config options are: %s %s %s" % (self.api_key, self.api_secret, self.station_id))

        # get the database parameters we need to function
        binding = d.get('data_binding', 'davishealthapi_binding')
        ...
        if dbcol != memcol:
            raise Exception('davishealthapi schema mismatch: %s != %s' % (dbcol, memcol))
    ...
    def get_data(self, now_ts, last_ts):
        record = GetAPIData.get_data(self.station_id, self.api_key, self.api_secret, self.archive_interval)
